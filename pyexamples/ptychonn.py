...
def conv_box_params(state: NetworkState, n_filters: int, end_new_block: bool = False, offset_type: str = None):
...
ens = NetworkState()
poolfn = partial(pool_params, ens)
...
arch_encoder = [  #
    ...
    #    to_Conv(**convfn(128)),
    #    connect_en(),
    #    to_Conv(**convfn(128, True)),
]

dns = copy.deepcopy(ens)
deconvfn = partial(conv_box_params, dns)
unpool_up_fn = partial(unpool_params, dns, "up")
connect_d1 = partial(connect, dns)

arch_decoder_1 = [  #
    # l0
    to_Conv(**deconvfn(n_filters=128, end_new_block=True, offset_type="first_up")),
    connect_d1(),
    # l1
    to_UnPool(**unpool_up_fn()),
    connect_d1(),
